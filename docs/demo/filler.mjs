...
const T = [
  ...
  ['node-drain','drain never finishes on one node','A pod with no controller cannot be evicted.','kubectl drain worker-3 --ignore-daemonsets','error: cannot delete Pods not managed by ReplicationController','A bare pod blocks the drain. Delete it, or pass --force deliberately.'],
  ['kubelet-cert','kubelet stops reporting after a reboot','Its client certificate expired while the node was down.','journalctl -u kubelet -n 2 --no-pager','x509: certificate has expired or is not yet valid','Rotation cannot run offline. Drop the stale cert and let the node re-bootstrap.'],
  ['reboot-window','patching reboots nodes during business hours','The maintenance window is in the wrong timezone.','systemctl cat unattended-upgrades.timer | grep OnCalendar','OnCalendar=*-*-* 02:00','02:00 is local to each node, not to the fleet. Pin the schedule to UTC.'],
  ['shutdown-hang','shutdown waits ten minutes on every reboot','One unit holds the shutdown inhibitor to its timeout.','systemd-analyze blame --no-pager | head -2','9min 58s csi-node.service','That unit ignores SIGTERM, so every reboot burns the full stop timeout.'],
  ['pod-restart','a pod restarts every few minutes after a node reboot','The liveness probe starts before the app is ready.','kubectl get pod api-0 -o jsonpath="{.status.containerStatuses[0].restartCount}"','37','No initialDelaySeconds, so a slow start reads as a failed probe.'],
